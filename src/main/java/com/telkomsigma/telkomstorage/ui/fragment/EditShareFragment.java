...
public class EditShareFragment extends Fragment {
    ...
    private void refreshUiFromState(View editShareView) {
        if (editShareView != null) {
            ...
            SwitchCompat shareSwitch = (SwitchCompat) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canShareSwitch);
            ...
            SwitchCompat switchCompat = (SwitchCompat) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditSwitch);
            ...
            if (mFile.isFolder() && areEditOptionsAvailable) {
                ...
                AppCompatCheckBox checkBox = (AppCompatCheckBox) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditCreateCheckBox);
                ...
                checkBox = (AppCompatCheckBox) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditChangeCheckBox);
                checkBox.setChecked((sharePermissions & OCShare.UPDATE_PERMISSION_FLAG) > 0);
                checkBox.setVisibility((canEdit) ? View.VISIBLE : View.GONE);
                ThemeUtils.tintCheckbox(checkBox, accentColor);

                checkBox = (AppCompatCheckBox) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditDeleteCheckBox);
    ...
    private void setPermissionsListening(View editShareView, boolean enable) {
        ...
        compound = (CompoundButton) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canShareSwitch);
        compound.setOnCheckedChangeListener(changeListener);

        compound = (CompoundButton) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditSwitch);
        compound.setOnCheckedChangeListener(changeListener);

        if (mFile.isFolder()) {
            compound = (CompoundButton) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditCreateCheckBox);
            compound.setOnCheckedChangeListener(changeListener);

            compound = (CompoundButton) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditChangeCheckBox);
            compound.setOnCheckedChangeListener(changeListener);

            compound = (CompoundButton) editShareView.findViewById(com.telkomsigma.telkomstorage.R.id.canEditDeleteCheckBox);
            compound.setOnCheckedChangeListener(changeListener);
        }
    ...
    private class OnPrivilegeChangeListener
                    ...
                    if (mFile.isFolder()) {
                        if (isChecked) {
                            if (!isFederated) {
                                ...
                                //noinspection ConstantConditions, prevented in the method beginning
                                subordinate = (CompoundButton) getView().findViewById(com.telkomsigma.telkomstorage.R.id.canEditDeleteCheckBox);
                                if (!subordinate.isChecked()) {
                                    toggleDisablingListener(subordinate);
                                }
        ...
        private void syncCanEditSwitch(View subordinateCheckBoxView, boolean isChecked) {
            CompoundButton canEditCompound = (CompoundButton) getView().findViewById(com.telkomsigma.telkomstorage.R.id.canEditSwitch);
            if (isChecked) {
